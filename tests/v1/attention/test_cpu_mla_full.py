...
def compute_hf_output(
    ...
    try:
        ...
        pytest.skip(f"transformers DeepseekV3RotaryEmbedding 不可用：{exc}")
    ...
    for i in range(batch_spec.batch_size):
        ...
        position_ids = torch.arange(s_len, device=device).unsqueeze(0)  # [1, s_len]
...
class _MockMLALayer(AttentionLayerBase):
    ...
    def __init__(
        ...
        # v0.21 引入了 MLAPrefillBackend，metadata builder 通过
        # ``static_forward_context[layer_name].prefill_backend`` 读取后端实例。
        # 在测试中手动挂载一个 CPUPrefillBackend，使 ``CPUMLAMetadataBuilder``
        # 能正常拿到。
        from vllm.config import get_current_vllm_config
        from vllm.v1.attention.backends.mla.prefill.cpu import CPUPrefillBackend

        self.prefill_backend = CPUPrefillBackend(
            num_heads=NUM_HEADS,
            scale=impl.scale,
            kv_lora_rank=KV_LORA_RANK,
            qk_nope_head_dim=QK_NOPE_HEAD_DIM,
...
def _run_report(args: argparse.Namespace) -> int:
    ...
    print("\n" + "=" * 92)
    print("完整 MLA 端到端对比：CPUMLAImpl vs transformers.DeepseekV3Attention（含真实 RoPE）")
    print("=" * 92)
    print(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="完整 MLA 端到端对比：CPUMLAImpl vs transformers.DeepseekV3Attention"
    )
    parser.add_argument(
        "--case",
        type=str,
